...
# --- CÀRREGA DE DADES AMB CACHE PER VELOCITAT ---
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_dades():
    try:
        # Resultats i Estadístiques en paral·lel
        r = requests.get(f"{BASE_URL}/results/primitiva?limit=50", headers=HEADERS, timeout=10).json()
        s = requests.get(f"{BASE_URL}/statistics/primitiva/numbers", headers=HEADERS, timeout=10).json()
